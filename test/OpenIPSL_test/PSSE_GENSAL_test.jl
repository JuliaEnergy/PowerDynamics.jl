...
if isdefined(Main, :EXPORT_FIGURES) && Main.EXPORT_FIGURES
    fig = let
        ...
        lines!(ax1, ref.time, ref[!, Symbol("gENSAL.w")]; label="OpenIPSL", color=Cycled(1), linewidth=2, alpha=0.7)
        lines!(ax1, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊w)).u; label="PowerDynamics", color=Cycled(1), linewidth=2, linestyle=:dash)
        axislegend(ax1)

        # Plot 2: Rotor angle δ
        ax2 = Axis(fig[1,2]; xlabel="Time [s]", ylabel="δ [rad]", title="Generator: Rotor Angle")
        lines!(ax2, ref.time, ref[!, Symbol("gENSAL.delta")]; label="OpenIPSL", color=Cycled(1), linewidth=2, alpha=0.7)
        lines!(ax2, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊delta)).u; label="PowerDynamics", color=Cycled(1), linewidth=2, linestyle=:dash)
        axislegend(ax2)

        # Plot 3: Active power P
        ax3 = Axis(fig[2,1]; xlabel="Time [s]", ylabel="P [pu]", title="Generator: Active Power")
        lines!(ax3, ref.time, ref[!, Symbol("gENSAL.P")]; label="OpenIPSL", color=Cycled(2), linewidth=2, alpha=0.7)
        lines!(ax3, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊P)).u; label="PowerDynamics", color=Cycled(2), linewidth=2, linestyle=:dash)
        axislegend(ax3)

        # Plot 4: Reactive power Q
        ax4 = Axis(fig[2,2]; xlabel="Time [s]", ylabel="Q [pu]", title="Generator: Reactive Power")
        lines!(ax4, ref.time, ref[!, Symbol("gENSAL.Q")]; label="OpenIPSL", color=Cycled(2), linewidth=2, alpha=0.7)
        lines!(ax4, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊Q)).u; label="PowerDynamics", color=Cycled(2), linewidth=2, linestyle=:dash)
        axislegend(ax4)

        # Plot 5: Terminal voltage Vt
        ax5 = Axis(fig[3,1]; xlabel="Time [s]", ylabel="Vt [pu]", title="Generator: Terminal Voltage")
        lines!(ax5, ref.time, ref[!, Symbol("gENSAL.Vt")]; label="OpenIPSL", color=Cycled(3), linewidth=2, alpha=0.7)
        lines!(ax5, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊Vt)).u; label="PowerDynamics", color=Cycled(3), linewidth=2, linestyle=:dash)
        axislegend(ax5)

        # Plot 6: Voltage angle
        ax6 = Axis(fig[3,2]; xlabel="Time [s]", ylabel="anglev [rad]", title="Generator: Voltage Angle")
        lines!(ax6, ref.time, ref[!, Symbol("gENSAL.anglev")]; label="OpenIPSL", color=Cycled(3), linewidth=2, alpha=0.7)
        lines!(ax6, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊anglev)).u; label="PowerDynamics", color=Cycled(3), linewidth=2, linestyle=:dash)
        axislegend(ax6)

        # Plot 7: State variables PSIkd and Epq
        ax7 = Axis(fig[4,1]; xlabel="Time [s]", ylabel="[pu]", title="Generator: State Variables")
        lines!(ax7, ref.time, ref[!, Symbol("gENSAL.PSIkd")]; label="OpenIPSL PSIkd", color=Cycled(4), linewidth=2, alpha=0.7)
        lines!(ax7, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊PSIkd)).u; label="PowerDynamics PSIkd", color=Cycled(4), linewidth=2, linestyle=:dash)
        lines!(ax7, ref.time, ref[!, Symbol("gENSAL.Epq")]; label="OpenIPSL Epq", color=Cycled(5), linewidth=2, alpha=0.7)
        lines!(ax7, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊Epq)).u; label="PowerDynamics Epq", color=Cycled(5), linewidth=2, linestyle=:dash)
        axislegend(ax7)

        # Plot 8: Currents id and iq
        ax8 = Axis(fig[4,2]; xlabel="Time [s]", ylabel="Current [pu]", title="Generator: dq Currents")
        lines!(ax8, ref.time, ref[!, Symbol("gENSAL.id")]; label="OpenIPSL id", color=Cycled(6), linewidth=2, alpha=0.7)
        lines!(ax8, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊id)).u; label="PowerDynamics id", color=Cycled(6), linewidth=2, linestyle=:dash)
        lines!(ax8, ref.time, ref[!, Symbol("gENSAL.iq")]; label="OpenIPSL iq", color=Cycled(7), linewidth=2, alpha=0.7)
        lines!(ax8, ts, sol(ts, idxs=VIndex(:GEN1, :gensal₊iq)).u; label="PowerDynamics iq", color=Cycled(7), linewidth=2, linestyle=:dash)
        axislegend(ax8)

        fig
    end
    save(joinpath(pkgdir(PowerDynamics),"docs","src","assets","OpenIPSL_valid","GENSAL.png"), fig)
end
