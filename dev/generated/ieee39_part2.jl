...
# > jump directly to the Initialize all Components section at the end.
#
...
#
# There are shortcut functions to do this (as shown later),
...
# We can use the function `powerflow_model`.
...
# 1. If the model has the `:pfmodel` metadata set (see Metadata), it will return the `VertexModel` stored in the metadata. In Part I we set the `:pfmodel` metadata using the `pf` keyword to the [`compile_bus`].
...
# With the default state, we can call `find_fixpoint`, which keeps $p$ constant and tries to find an $x$ such that
...
# `initialize_component` solve for the rest:
...
# >
# > The non mutating `initialize_component` returns a dictionary
...
# Recalling from Part 1, we have an uncontrolled machine
...
# The fact that the model is structurally underconstrained is a property of the model and should therefore be handled by the model. To do so, NetworkDynamics.jl provides the `InitFormula` mechanism.
#
...
#
# See NetworkDynamics docs on Advanced Component Initialization: Formulas and Constraints and the PowerDynamics specific extension
# Advanced Component Initialization for more information on method 2 and 3.
#
...
#
# We start by with `solve_powerflow`, which extracts the powerflow models from each component,
...
# state. Those `interface_values` are the shared state between the power flow
...
# solution). For that we use `initialize_componentwise!`.
...
# We can just use `initialize_from_pf!` to do everything from
