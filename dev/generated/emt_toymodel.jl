...
# `compile_bus` call — see the IEEE 9-Bus Example for the per-bus form.
#
...
# `Zbase` and `Ybase`. Note what the *dynamic* models below want: not an
...
# > **Library equivalent**
# >
# > Apart from the abc back-transform, this is exactly `DynamicCShunt` from the
...
# > **Library equivalent**
# >
# > This model is `DynamicSeriesRLBranch` without the transformer ratios; the library
...
# Powergrid Initialization for the full picture. The part we have to supply is the
...
# call. `initialize_from_pf` solves the power flow, initializes every component against
