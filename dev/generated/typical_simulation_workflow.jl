...
#
# **First**: we define an "injector model", in this case our
# inverter:
# ```asciiart
# (t) ┌────────────────┐
#  o──┤ Droop Inverter │
...
#
# **Second**: we build a "Bus Model", which connects
...
# Doing so, we get a `VertexModel`, which is an
...
# For more information on the different model types, see the Modeling Concepts docs.
# You can check out the NetworkDynamics.jl doc on the underlying mathematical model.
#
...
# Now we can build the network using the `Network` constructor from NetworkDynamics.jl
...
# `systembase₊Sbase`/`ωbase`/`ωframe`); see Modeling Concepts for what they mean.
#
...
# We do so using `initialize_from_pf`:
...
#
# The return value of the `initialize_from_pf` function is a so-called `NWState` object,
...
# Indexing docs of ND.jl and especially the
# `FilteringProxy` for more details.
#
...
# DiffEq.jl docs. Specific extensions for NetworkDynamics.jl can be found in the
# NetworkDynamics.jl callback docs.
#
