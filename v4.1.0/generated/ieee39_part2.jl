...
# 1. If the model has the `:pfmodel` metadata set (see Metadata), it will return the `VertexModel` stored in the metadata. In Part I we set the `:pfmodel` metadata using the `pf` keyword to the [`Bus`].
