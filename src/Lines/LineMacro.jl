...
macro Line(typedef, func_body)
    return create(typedef, func_body)
end

function create_line(name, parameters, func_body)
    ...
    es_real = [:(push!(e, real(current_vector[1])))]
    es_imag = [:(push!(e, imag(current_vector[1])))]
    ed_real = [:(push!(e, real(current_vector[2])))]
    ed_imag = [:(push!(e, imag(current_vector[2])))]
...
function create(typedef, func_body)
    @capture(typedef, name_(parameters__))
    mainex = create_line(name, parameters, func_body)
