@DynamicNode GridFormingTecnalia(ω_r,τ_U, τ_I, τ_V, τ_ω, τ_P, τ_Q, n_P, n_Q, K_P, K_Q, P, Q, V_r, R_f, X_f) begin
    MassMatrix(m_u = true, m_int = [true,true,true,true,true,true,true])
end begin
    @assert τ_U >= 0
    @assert τ_I >= 0
    @assert τ_P >= 0
    ...
    @assert R_f >= 0
    @assert X_f >= 0
end [[u_fil_r,du_fil_r],[u_fil_i,du_fil_i],[i_fil_r,di_fil_r],[i_fil_i,di_fil_i],[v_fil,dv_fil],[ω_fil,dω_fil],[ω, dω]] begin
    ...
    dv_fil = 1/τ_V*(-v_fil + V_r)
    dω_fil = 1/τ_ω*(-ω_fil + ω_r)

    dω = 1/τ_P*(ω_fil-ω) + K_P/τ_P*(P-p) - K_P/n_P*dp
    v = abs(u_fil)
    dv = 1/τ_Q*(v_fil-v) + K_Q/τ_Q*(Q-q) - K_Q/n_Q*dq
