...
function plot_res(result, powergrid,disturbed_node)
    ...
    #append!(ω_indices,findall(n -> isa(n, CurtailedPowerPlantWithInertia1), powergrid.nodes))
    ...
    p_labels = reshape([latexstring(string(raw"p", "_{$i}")) for i in collect(keys(powergrid.nodes))], (1, length(powergrid.nodes)))
    println(p_labels)
