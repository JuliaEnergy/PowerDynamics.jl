...
function plot_res(result, powergrid,disturbed_node)
    ...
    pl_p = plot(result, :, :p, ylims=(-20.,30.), legend = (0.8, 0.95), ylabel=L"p [p.u.]", label=p_labels)
    pl_q = plot(result, :, :q, ylims=(-20.,20.), legend = (0.8, 0.95), ylabel=L"q [p.u.]", label=p_labels)
    pl_v = plot(result, :, :v, ylims=(0.9,1.05), legend = (0.8, 0.95), ylabel=L"v [p.u.]", label=p_labels)
    pl_ω = plot(result, ω_indices, :ω, legend = (0.8, 0.7), ylims = (-0.5,0.5), ylabel=L"\omega \left[rad/s\right]", label=ω_labels)#, color=ω_colors, left_margin = 4mm)
    pl = plot(
        pl_ω, pl_v, pl_p, pl_q;
        layout=(2,2),
        size = (1000, 700),
