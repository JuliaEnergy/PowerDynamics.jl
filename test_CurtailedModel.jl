...
node_list=[]
    ...
    append!(node_list, [SwingEqLVS(H=5, P=-0.1, D=0.1, Ω=50,Γ=0.1,V=1.)])
    ...
    append!(node_list, [CurtailedPowerPlantWithInertia(PICFunction=true,P=0.5,ω_0=2π*50,T_AI=10,K_PPLL=50,K_IPLL=625,T_d=0.003,T_f=0.004,K_PV=0.28,K_IV=186)])
    ...
    line_list=[]
    # take PI model instead
    append!(line_list,[PiModelLine(from=1, to=2, y=-1im/0.02, y_shunt_km=0, y_shunt_mk=0)])
    append!(line_list,[PiModelLine(from=2, to=3, y=-1im/0.02, y_shunt_km=0, y_shunt_mk=0)])
...
sol = solve(problem,Rodas4(autodiff=false),saveat=1e-6)#,callback=cb)
