...
# MTKModel. This model should fulfill the Branch Interface, i.e. it needs to have two `Terminal`, one called
...
# Their general capability is extended in NetworkDynamics.
#
...
# Once the model is defined, we can go through the building hierarchy outlined in Modeling Concepts.
# First, we need to form something satisfying the MTKLine Interface.
#
...
# Then, we take the mtkline and put it into a compiled `EdgeModel` by
# calling the `compile_line` constructor
# ```asciiart
#
#        ╔═══════════════════════════════════════════════╗
#        ║ EdgeModel (compiled)                          ║
...
#
# For `ComponentCondition`, we need to specify
...
# The third part of the IEEE39 Grid Tutorial simulates a short circuit on a line.
...
# Also, we initialize the model (the quintessence of part II).
...
# initialization.) `check_base_consistency`, which `initialize_from_pf!` runs for us,
...
# we do so by extracting the `interface_values`
